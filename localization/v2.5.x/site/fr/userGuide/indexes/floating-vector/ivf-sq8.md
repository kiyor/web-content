...
<li><p><strong>Normalisation :</strong> Normaliser les valeurs du vecteur dans une plage comprise entre 0 et 1 à l'aide de la formule :</p>
<p>$$ \text{valeur_normalisée} = \frac{\text{valeur} - \text{min}}{\text{max} - \text{min}} $$</p>
<p>Cela permet de s'assurer que toutes les valeurs sont mappées proportionnellement dans une plage normalisée, les préparant ainsi à la compression.</p></li>
<li><p><strong>Compression 8 bits :</strong> Multiplier la valeur normalisée par 255 (la valeur maximale d'un entier de 8 bits) et arrondir le résultat à l'entier le plus proche. Cela permet de compresser chaque valeur en une représentation de 8 bits.</p></li>
</ol>
