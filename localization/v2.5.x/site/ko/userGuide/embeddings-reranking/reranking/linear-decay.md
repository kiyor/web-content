...
<p>S(doc) = \max\left( \frac{s - \max(0, |fieldvalue_{doc} - origin| - offset)}{s}, 0 \right) $$입니다.</p>
<p>여기서</p>
<p>여기서: $$ s = \frac {scale}{(1.0 - decay)} $$</p>
...
<li><p>스케일과 감쇠 값에서 파라미터 $s$를 결정합니다.</p></li>
<li><p>조정된 거리를 $s$에서 빼고 $s$로 나눕니다.</p></li>
<li><p>결과가 0 이하로 내려가지 않도록 합니다: $\max(결과, 0)$.</p></li>
</ol>
<p>s$ 계산은 스케일과 감쇠 매개변수를 점수가 0에 도달하는 지점으로 변환합니다. 예를 들어, 감쇠가 0.5이고 스케일이 7이면 점수는 거리=14(스케일 값의 두 배)에서 정확히 0에 도달합니다.</p>
