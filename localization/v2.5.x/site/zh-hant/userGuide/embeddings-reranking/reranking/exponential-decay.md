...
<li><p>乘以 $\lambda$，這是根據您的比例和衰減參數計算出來的。</p></li>
<li><p>取指數，得出介於 0 和 1 之間的值：$\exp(\lambda \cdot value)$。</p></li>
</ol>
<p>$\lambda$ 計算將您的比例和衰減參數轉換為指數函數的速率參數。較負的 $\lambda$ 會產生較陡的初始下降。</p>
<h2 id="Use-exponential-decay" class="common-anchor-header">使用指數衰減<button data-href="#Use-exponential-decay" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在 Milvus 中，指數遞減可以應用於標準向量搜索和混合搜索操作。以下是實現此功能的關鍵程式碼片段。</p>
