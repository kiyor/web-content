...
<p>AIGC 應用程式通常使用向量資料庫作為知識庫，以管理使用者與大型語言模型 (LLM) 互動期間所產生的資料。這樣的知識庫幾乎大同小異。為了加速 Milvus 叢集在這種情況下的使用，有一個即時方法可供您建立一個只有兩個參數的集合，即集合名稱和向量字段維度。</p>
<p>當您使用預設值即時建立集合時，下列設定將會適用：</p>
<ul>
<li><p>主欄位和向量欄位會新增至模式<strong>（id</strong>和<strong>向量</strong>）。</p></li>
