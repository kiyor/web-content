...
<h3 id="Graph-based-index-memory-usage" class="common-anchor-header">グラフベースのインデックスのメモリ使用量</h3><p>HNSWのようなグラフベースのインデックス・タイプは、グラフ構造と生のベクトル埋め込みを保存するために大きなメモリを必要とします。以下は、HNSWインデックスタイプを使用してインデックス付けされた128次元ベクトル100万個が消費するメモリの詳細な内訳です。</p>
<ol>
