...
<h2 id="Formula" class="common-anchor-header">計算式<button data-href="#Formula" class="anchor-icon" translate="no">
    ...
    </button></h2><p>線形減衰スコアを計算する数式は以下の通りである：</p>
<p>S(doc) = Γmaxleft( Γfrac{s - Γmax(0, |fieldvalue_{doc} - origin| - offset)}{s}, 0 Γright) $$ である。</p>
<p>ここで</p>
<p>s = Γ{scale}{(1.0 - decay)} $$ です。</p>
<p>これをわかりやすく分解すると</p>
<ol>
<li><p>フィールドの値が原点からどのくらい離れているかを計算します： $|fieldvalue_{doc} - origin|$.</p></li>
<li><p>もしあれば）オフセットを引くが、決してゼロ以下にはしない： $max(0, distance - offset)$.</p></li>
<li><p>scale と decay の値からパラメータ $s$ を決める。</p></li>
<li><p>s$から調整した距離を引いて$s$で割る。</p></li>
<li><p>結果がゼロ以下にならないようにする：$max(result, 0)$.</p></li>
</ol>
<p>s$の計算は、スケールと減衰のパラメータをスコアがゼロになるポイントに変換する。例えば、decay=0.5、scale=7 の場合、distance=14（scale の 2 倍）でスコアがちょうど 0 になる。</p>
