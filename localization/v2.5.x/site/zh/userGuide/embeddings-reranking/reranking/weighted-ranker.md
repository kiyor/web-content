...
title: 加权排名器Compatible with Milvus 2.6.x
summary: >-
...
beta: Milvus 2.6.x
---
<h1 id="Weighted-Ranker" class="common-anchor-header">加权排名器<span class="beta-tag" style="background-color:rgb(0, 179, 255);color:white" translate="no">Compatible with Milvus 2.6.x</span><button data-href="#Weighted-Ranker" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>如果您的混合搜索应用需要结合多种搜索路径，同时控制它们的相对重要性，加权排名器就是您的理想选择。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Apply-to-hybrid-search" class="common-anchor-header">应用于混合搜索</h3><p>加权排名器专为结合多个向量场的混合搜索操作而设计。执行混合搜索时，必须为每条搜索路径指定权重：</p>
<div class="multipleCode">
